...
from models import load_llm, load_tokenizer,get_models
import utils 
...
def extract_llm_features(filenames, dataset, args):
    ...
    从语言模型中提取特征。
    参数：
    filenames: 由Huggingface标识符表示的语言模型名称列表
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path",       type=str, default="llm_model")
    args = parser.parse_args()

    if args.qlora:
        print(f"QLoRA is set to True. The alignment score will be slightly off.")

    llm_models, lvm_models = get_models(args.modelset, args.model_path,modality=args.modality)
